...
using UzEx.Dapper.Oracle.BulkSql;
using UzEx.Dapper.Oracle.Tests.IntegrationTests.Util;
using Xunit;

namespace UzEx.Dapper.Oracle.Tests.IntegrationTests.BulkSql
{
    [Collection("OracleDocker")]
    public class BulkOperationTests
    {
        private const string InsertSql =
            "Insert into BULKCUSTOMERS (CUSTOMERID,COMPANYNAME,CITY,CONTACTNAME,CONTACTTITLE,ADDRESS,POSTALCODE,COUNTRY,PHONE,FAX,TIDSSTEMPEL,OPPRETTETAV,OPPRETTETTID,SISTENDRETAV,SISTENDRETTID,DIPSID) " +
        ...
        public void BulkSql()
        {
            ...
            foreach (var customer in customers) customer.CustomerId = Guid.NewGuid();
        ...
        private IEnumerable<BulkMapping<Customer>> CreateMapping()
        {
            yield return new BulkMapping<Customer>("CUSTOMERID", c => c.CustomerId.ToByteArray(),
                OracleMappingType.Raw);
        ...
        {
            using (var s = GetType().Assembly
                .GetManifestResourceStream("Tests.Dapper.Oracle.IntegrationTests.BulkSql.customers.json"))
            {
    ...
    public class CustomerDAL
    {
        ...
        {
            var insertSql =
                "INSERT INTO CUSTOMERS(CUSTOMERID,NAME,ADDRESS,POSTALCODE,CITY) VALUES(:CUSTOMERID,:NAME,:POSTALCODE,:CITY)";
