    ...
    public class DatabaseFixture : IDisposable, IAsyncLifetime
    {
                ...
                proccess.OutputDataReceived += delegate(object sender, DataReceivedEventArgs args)
                {
                    if (!string.IsNullOrEmpty(args.Data))
                        Console.WriteLine(args
                            .Data); //TODO: Figure out how to get data from powershell proccess into test logs...
                };
                while (!proccess.HasExited) await Task.Delay(TimeSpan.FromSeconds(1));

                if (proccess.ExitCode != 0)
                    throw new Exception("Error during bootstrap.  Read readme.txt and check container state manually");

                connectionString =
                    "Data Source=localhost/ORCLPDB1.localdomain;User Id=SYS;Password=Oradoc_db1;DBA Privilege=SYSDBA";
            }
        ...
        private static string GetBootstrapFolder()
        {
            Func<string, bool> folderPredicate =
                s => File.Exists(Path.Combine(s, "scripts", "LocalOracleDockerDb.ps1"));

            var folder = new DirectoryInfo(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location));
            while (folder.Parent != null && !folderPredicate(folder.FullName)) folder = folder.Parent;
