...
namespace UzEx.Dapper.Oracle
{
    public static class OracleValueConverter
    {
        ...
        public static T Convert<T>(object value)
        {
            ...
            if (value is IConvertible) return (T) System.Convert.ChangeType(value, nullableType ?? typeof(T));
        ...
        /// <summary>
        ///     OracleString[] does not implement IConvertible therefore this is the only way to convert OracleString[] to string[]
        ///     or OracleDecimal[] to .NET native data type
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="value"></param>
        /// <returns></returns>
        private static T ConvertArray<T>(object value)
        {
            ...
            switch (typeof(T).FullName)
            {
                ...
                case "System.Int16[]":
                    ...
                    for (var i = 0; i < arr.Length; i++) shortArray[i] = short.Parse(arr.GetValue(i).ToString());
                    return (T) System.Convert.ChangeType(shortArray, nullableType ?? typeof(T));

                case "System.Int32[]":
                    var intArray = new int[arr.Length];
                    for (var i = 0; i < arr.Length; i++) intArray[i] = int.Parse(arr.GetValue(i).ToString());
                    return (T) System.Convert.ChangeType(intArray, nullableType ?? typeof(T));

                case "System.Int64[]":
                    var longArray = new long[arr.Length];
                    for (var i = 0; i < arr.Length; i++) longArray[i] = long.Parse(arr.GetValue(i).ToString());
                ...
                case "Oracle.ManagedDataAccess.Types.OracleDecimal[]":
                    ...
                    for (var i = 0; i < arr.Length; i++) decimalArray[i] = decimal.Parse(arr.GetValue(i).ToString());
                    return (T) System.Convert.ChangeType(decimalArray, nullableType ?? typeof(T));

                default:
                    var strArray = new string[arr.Length];
                    for (var i = 0; i < arr.Length; i++) strArray[i] = arr.GetValue(i).ToString();
        ...
        private static object GetValue(object value)
        {
            ...
            while (value is DbParameter parameter) value = parameter.Value;
            ...
            if (IsOracleDataStructure(valueType))
            {
                ...
                if (isNullProp != null)
                {
                    ...
                    if (propValue != null) isNull = (bool) propValue;
                }
