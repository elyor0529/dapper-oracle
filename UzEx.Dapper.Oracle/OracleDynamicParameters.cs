...
namespace UzEx.Dapper.Oracle
{
    ...
    public class OracleDynamicParameters : SqlMapper.IDynamicParameters
    {
        ...
        {
            if (ArrayBindCount > 0) OracleMethodHelper.SetArrayBindCount(command, ArrayBindCount);

            if (InitialLOBFetchSize > 0) OracleMethodHelper.SetInitialLobFetchSize(command, InitialLOBFetchSize);

            if (BindByName) OracleMethodHelper.SetBindByName(command, BindByName);
